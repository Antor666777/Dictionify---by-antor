...
let append_show_more_button_here = document.querySelectorAll('#append_show_more_button_here')[0]
let isRunnedAlready = false
const get_definitions_main_api = async(word) => {
  let definitions = {}
  let response_status_code = 0
  await fetch(`https://dictionary-by-api-ninjas.p.rapidapi.com/v1/dictionary?word=${word}`,{
    method: 'GET',
    headers: {
...
const get_definitions_second_api = async(word) => {
  ...
  await fetch(`https://mashape-community-urban-dictionary.p.rapidapi.com/define?term=${word}`, options).then((response)=>{
    return response.json()
  }).then((value)=>{
...
const onSearchClick = async(event) => {
  ...
  let definition_search_input = document.getElementById('definition_search_input')
  let definitions = await get_definitions_main_api(definition_search_input.value)
  ...
  if(definitions.valid == false){
    // showErrorDialogue()
    let definition_seconds_api = await get_definitions_second_api(definition_search_input.value)
    definition_seconds_api = definition_seconds_api.list
    // console.log(definition_seconds_api);
    if(definition_seconds_api.length == 0 || definition_seconds_api.length == undefined || definition_seconds_api.length == null){
      showErrorDialogue()
    }
    else{
      // console.log(definition_seconds_api);
      show_message(definition_search_input.value, definition_seconds_api[0].definition)
      let show_more_button = document.querySelectorAll('#show_more_button')[0]
      if(show_more_button == undefined || show_more_button == null){
        if(definition_seconds_api.length > 1){
          await create_show_more(append_show_more_button_here,'Show more','show_more_button')
          let show_more_button = document.querySelectorAll("#show_more_button")[0]
